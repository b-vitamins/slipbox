...
The critical condition emerges from the transfer matrix analysis [[7.7]]. Random walks proliferate with fugacity \(t = \tanh K\) per step, while the number of \(\ell\)-step walks from a given origin grows as \((2d)^\ell\) on a \(d\)-dimensional hypercubic lattice. The partition function for phantom loops diverges when
