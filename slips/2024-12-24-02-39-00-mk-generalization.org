...
The Migdal-Kadanoff method generalizes elegantly to arbitrary statistical models through the transfer matrix formalism [[id:8d5c9e3f-7b4a-6f2c-9e8d-7b4a6f2c9e8d][Transfer Matrix Formalism]]. For any one-dimensional system with interactions {K}, the transfer matrix T({K}) generates partition functions through its powers. The MK prescription in d dimensions becomes:
...
For the Potts model [[id:1e9d8f3c-7a5b-6f2c-8e9d-7a5b6f2c8e9d][Potts Model]], the transfer matrix elements are:
...
For O(n) models [[id:2f8d9c3e-6a5b-7f2c-9d8e-6a5b7f2c9d8e][O(n) Models]], the transfer operator acts on functions on the (n-1)-sphere:

\[\langle\vec{S}_1|T|\vec{S}_2\rangle = \exp[K\vec{S}_1 \cdot \vec{S}_2 + \vec{h} \cdot (\vec{S}_1 + \vec{S}_2)/2]\]

The spherical harmonic expansion [[id:3f9d8c3e-7a6b-5f2c-8e9d-7a6b5f2c8e9d][Spherical Harmonics]] enables analytical progress, though explicit recursion relations exist only for specific n values.
