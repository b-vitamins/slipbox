...
Starting from the exact partition function $Z = 2^N\cosh^{dN}K \times S$ where $S$ sums over allowed graphs [[#7.35]], we introduce the generating function $\Xi$ for single loops [[#7.36]]. The crucial approximation exponentiates this quantity:
...
The factor $1/\ell$ accounts for the $\ell$ equivalent starting points on a loop of length $\ell$, ensuring proper extensivity. This sum over phantom loops reduces to a trace over transfer matrix powers [[#7.23]], yielding closed-form expressions in momentum space.
